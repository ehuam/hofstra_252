...
A smart compiler can also recognize these cases of recursion and
compile them to efficiently excuting code where recursive calls do not
push new frames onto the runtime call stack.  The gcc and g++
compilers both do this at level 2 or above optimization (use gcc with
-O2).  The scheme interpreter certainly do this.  Be aware however,
that Java, Python, C# and many other languages do NOT optimize tail
recursion.  One reason for this is that it becomes harder to implement
it along with dynamic dispatch, where the version of the function
being called is not determined at compile time.
...
This program in C, with scheme code in comments, shows you how to convert
a program that uses while loops into a tail-recursive program.
